...
<form id="form">                    
    ...
    <div class="mb-1">                        
        <label for="input-uraian">Penerima</label>
    ...
    <script>
        $(function(){
            var table = $("#grid").DataTable({
                ...
                {
                    $(row).attr("row-id", data.penerima_id);
                },
                buttons: [{
                text: 'Tambah',
                ...
                action: function (e, dt) {
                    ...
                    $("#input-id").val(row[0].penerima_id);
                    $("#modalform").modal("show");
                }
                },
                {
                ...
                action: function (e, dt){
                    ...
                    $("#modal .btn-ok").html("Ya").on("click", function(){
                        ...
                        $.ajax({
                            url: "/master/crud",
                            data: {_token: "{{ csrf_token() }}", action: "penerima", input: $.param({"input-action": "delete", "id": row[0].penerima_id})},
                            type: "POST",
            ...
            $("#saveform").on("click", function(){
                ...
                $.ajax({
                    url: "/master/crud",
                    data: {_token: "{{ csrf_token() }}", action: "penerima", input: $("#form").serialize()},
                    type: "POST",
                    success: function(msg) {
                        if (typeof msg.error != 'undefined'){
                            $("#modal .modal-body").html(msg.error);
