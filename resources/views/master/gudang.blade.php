    ...
    <script>
        $(function(){
            var table = $("#grid").DataTable({
                ...
                {
                    $(row).attr("row-id", data.GUDANG_ID);
                },
                buttons: [{
                text: 'Tambah',
                ...
                action: function (e, dt){
                    ...
                    $("#modal .btn-ok").html("Ya").on("click", function(){
                        ...
                        $.ajax({
                            url: "/master/crud",
                            data: {_token: "{{ csrf_token() }}", action: "gudang", input: $.param({"input-action": "delete", "id": row[0].GUDANG_ID})},
                            type: "POST",
            ...
            $("#saveform").on("click", function(){
                ...
                $.ajax({
                    url: "/master/crud",
                    data: {_token: "{{ csrf_token() }}", action: "gudang", input: $("#form").serialize()},
                    type: "POST",
                    success: function(msg) {
                        if (typeof msg.error != 'undefined'){
                            $("#modal .modal-body").html(msg.error);
