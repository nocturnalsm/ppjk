...
<form id="form">
    ...
    <div class="mb-1">
        ...
        <input type="text" id="input-code" name="input-code" class="form-control validate">
    </div>
    <div class="mb-1">
...
@push('scripts_end')
    <script>
        $(function(){
            ...
            {
                $(row).attr("row-id", data.id_customer);
            },
            buttons: [{
            text: 'Tambah',
            ...
            action: function (e, dt) {
                ...
                $("#input-negara").val(row[0].negara_customer);
                $("#input-nama").val(row[0].nama_customer);
                $("#input-alamat").val(row[0].alamat_customer);
                $("#input-telepon").val(row[0].telp_customer);
                $("#input-fax").val(row[0].fax_customer);
                $("#input-link").val(row[0].link_customer);
                $("#input-code").val(row[0].code_customer);
                $("#input-action").val("edit");
                $("#input-id").val(row[0].id_customer);
                $("#modalform").modal("show");
            }
            },
            {
            ...
            action: function (e, dt){
                ...
                $("#modal .btn-ok").html("Ya").on("click", function(){
                    ...
                    $.ajax({
                        url: "/master/crud",
                        data: {_token: "{{ csrf_token() }}", action: "customer", input: $.param({"input-action": "delete", "id": row[0].id_customer})},
                        type: "POST",
        ...
        $("#saveform").on("click", function(){
            ...
            $.ajax({
                url: "/master/crud",
                data: {_token: "{{ csrf_token() }}", action: "customer", input: $("#form").serialize()},
                type: "POST",
                success: function(msg) {
                    if (typeof msg.error != 'undefined'){
                        $("#modal .modal-body").html(msg.error);
