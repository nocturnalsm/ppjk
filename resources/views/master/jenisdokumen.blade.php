...
<form id="form">                    
    ...
    <div class="mb-1">                        
        <label for="input-uraian">Jenis Dokumen</label>
    ...
    <script>
        $(function(){
            var table = $("#grid").DataTable({
                "processing": false,
                "serverSide": true,
                "ajax": "/master/getdata_jenisdokumen",
                "paging": false,
                ...
                {
                    $(row).attr("row-id", data.jenisdokumen_id);
                },
                buttons: [{
                text: 'Tambah',
                ...
                action: function (e, dt) {
                    ...
                    $("#input-id").val(row[0].jenisdokumen_id);
                    $("#modalform").modal("show");
                }
                },
                {
                ...
                action: function (e, dt){
                    ...
                    $("#modal .btn-ok").html("Ya").on("click", function(){
                        ...
                        $.ajax({
                            url: "/master/crud",
                            data: {_token: "{{ csrf_token() }}", action: "jenisdokumen", input: $.param({"input-action": "delete", "id": row[0].jenisdokumen_id})},
                            type: "POST",
            ...
            $("#saveform").on("click", function(){
                ...
                $.ajax({
                    url: "/master/crud",
                    data: {_token: "{{ csrf_token() }}", action: "jenisdokumen", input: $("#form").serialize()},
                    type: "POST",
                    success: function(msg) {
                        if (typeof msg.error != 'undefined'){
                            $("#modal .modal-body").html(msg.error);
