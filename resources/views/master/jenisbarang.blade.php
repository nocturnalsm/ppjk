    ...
    <script>
        $(function(){
            var table = $("#grid").DataTable({
                "processing": false,
                "serverSide": true,
                "ajax": "/master/getdata_jenisbarang",
                "paging": false,
                ...
                {
                    $(row).attr("row-id", data.jenisbarang_id);
                },
                buttons: [{
                text: 'Tambah',
                ...
                action: function (e, dt) {
                    ...
                    $("#input-id").val(row[0].jenisbarang_id);
                    $("#modalform").modal("show");
                }
                },
                {
                ...
                action: function (e, dt){
                    ...
                    $("#modal .btn-ok").html("Ya").on("click", function(){
                        ...
                        $.ajax({
                            url: "/master/crud",
                            data: {_token: "{{ csrf_token() }}", action: "jenisbarang", input: $.param({"input-action": "delete", "id": row[0].jenisbarang_id})},
                            type: "POST",
            ...
            $("#saveform").on("click", function(){
                ...
                $.ajax({
                    url: "/master/crud",
                    data: {_token: "{{ csrf_token() }}", action: "jenisbarang", input: $("#form").serialize()},
                    type: "POST",
                    success: function(msg) {
                        if (typeof msg.error != 'undefined'){
                            $("#modal .modal-body").html(msg.error);
